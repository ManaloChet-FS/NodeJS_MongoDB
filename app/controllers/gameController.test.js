...
describe("Query string and select tests", () => {
  test("Query string of title=Left 4 Dead", async () => {
    const result = await request(app).get("/api/v1/games?title=Left 4 Dead");
    const { data } = result.body;
    expect(data[0].title).toEqual("Left 4 Dead");
  });

  test("Select test of title and averageScore", async () => {
    const result = await request(app).get(
      "/api/v1/games?select=title,averageScore"
    );
    const { data } = result.body;
...
describe("Pagination tests", () => {
  test("Skip test", async () => {
    ...
    expect(data[0].title).toEqual("Elden Ring");
  })
